            ...
            <div class="col-8">
                <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('started_at') is-invalid @enderror" type="datetime-local" min="{{ now()->subMonths(5)->format('Y-m-d\TH:i') }}" max="{{ now()->addMonths(5)->format('Y-m-d\TH:i') }}" name="started_at" value="{{ old('started_at', $event->started_at) }}" id="started_at"/>
                        @error('started_at')
                            <p class="invalid-feedback"> {{ $message }} </p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control @error('end_at') is-invalid @enderror" type="datetime-local" min="{{ now()->subMonths(5)->format('Y-m-d\TH:i') }}" max="{{ now()->addMonths(5)->format('Y-m-d\TH:i') }}" name="end_at" value="{{ old('end_at', $event->end_at) }}" id="end_at"/>
                        @error('end_at')
                            <p class="invalid-feedback"> {{ $message }} </p>
                        @enderror
                    </div>
